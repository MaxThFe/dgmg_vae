...
class GraphEmbed(nn.Module): #TODO Change embedding?
...
class GraphProp(nn.Module):
    # Should be irrelevant, only for intermediate_graph representation
...
class AddNode(nn.Module):
    def __init__(self, graph_embed_func, node_hidden_size):
        ...
        self.stop = 1
        self.add_node = nn.Linear(graph_embed_func.graph_hidden_size*2, 1)

        # If to add a node, initialize its hv
        self.node_type_embed = nn.Embedding(1, node_hidden_size)
        self.initialize_hv = nn.Linear(node_hidden_size + \
                                       graph_embed_func.graph_hidden_size*2,
                                       node_hidden_size)

        self.init_node_activation = torch.zeros(1, 2 * node_hidden_size)

    def _initialize_node_repr(self, g, node_type, graph_embed, latent_z):
        num_nodes = g.number_of_nodes()
        hv_init = self.initialize_hv(
            torch.cat([
                self.node_type_embed(torch.LongTensor([node_type])),
                graph_embed, latent_z], dim=1))
    ...
    def forward(self, g, latent_z, action=None):
        graph_embed = self.graph_op['embed'](g)

        #logit = self.add_node(graph_embed)
        logit = self.add_node(torch.concat([graph_embed,latent_z],dim=1))
        prob = torch.sigmoid(logit)

        if not self.training:
            action = Bernoulli(prob).sample().long().item()
        stop = bool(action == self.stop)

        if not stop:
            g.add_nodes(1)
            self._initialize_node_repr(g, action, graph_embed, latent_z)
...
class AddEdge(nn.Module):
    def __init__(self, graph_embed_func, node_hidden_size):
        ...
        self.graph_op = {'embed': graph_embed_func}
        self.add_edge = nn.Linear(graph_embed_func.graph_hidden_size*2 + \
                                  node_hidden_size, 1)
    ...
    def forward(self, g, latent_z ,action=None):
        ...
        logit = self.add_edge(torch.cat(
            [graph_embed, latent_z, src_embed], dim=1))
        prob = torch.sigmoid(logit)
...
class DGMG(nn.Module):
    ...
    def init_weights(self):
        from decoder.utils import weights_init, dgmg_message_weight_init
    ...
    def add_node_and_update(self, a=None, z=None):
        """Decide if to add a new node.
        If a new node should be added, update the graph."""

        return self.add_node_agent(self.g, z, a)

    def add_edge_or_not(self,z, a=None):
        """Decide if a new edge should be added."""

        return self.add_edge_agent(self.g, z ,a)
    ...
    def forward_train_decoder(self, actions, latent_z):
        self.prepare_for_train()

        stop = self.add_node_and_update(a=actions[self.action_step],z=latent_z)

        while not stop:
            to_add_edge = self.add_edge_or_not(latent_z , a=actions[self.action_step])
            while to_add_edge:
                self.choose_dest_and_update(a=actions[self.action_step]) # TODO: check if latent z needed
                to_add_edge = self.add_edge_or_not(a=actions[self.action_step],z=latent_z)
            stop = self.add_node_and_update(a=actions[self.action_step],z=latent_z)

        return self.get_log_prob()

    def forward_inference(self,latent_z):
        stop = self.add_node_and_update(z=latent_z)
        while (not stop) and (self.g.number_of_nodes() < self.v_max + 1):
            num_trials = 0
            to_add_edge = self.add_edge_or_not(latent_z)
    ...
    def forward(self, actions=None, latent_z=None):
        ...
        if self.training:
            return self.forward_train_decoder(actions, latent_z)
        else:
            return self.forward_inference(latent_z)
