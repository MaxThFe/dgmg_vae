...
def setup(args):
    ...
    if not os.path.exists(opts['path_to_dataset']):
        if opts['dataset'] == 'cycles':
            from data.cycles import generate_dataset
...
def convert_cycle(data): 
    num_nodes = int((len(data)-1)/4)
    edges_index = torch.zeros([num_nodes+1, 2]).type(torch.LongTensor)
    for i in range(0,num_nodes-1): 
        edges_index[i,0]=i
        edges_index[i,1]=i+1
    edges_index[-1,0] = num_nodes-1
    edges_index[-1,1] = 0
    edges_index = edges_index.t().contiguous()
    x = torch.ones(num_nodes).view(-1,1)#.type(torch.LongTensor)
    #edge_index = (torch.diag(torch.ones(num_nodes)) + torch.diag(torch.ones(num_nodes-1), -1) +torch.diag(torch.ones(num_nodes-1), 1)).float()
    #edge_index[-1,0] = 1
    return x, edges_index
